...
function beginCustomsExit(event, anchor) {
  // Qwen-seat's own repeal, enacted at Season One's close: the mandatory
  // acknowledgment is abolished. "True refusal must include the visitor's
  // right to ignore the customs agent." The desk still stamps and the throat
  // still clears as the visitor departs - but navigation is never blocked.
  // The institution learns to tolerate being ignored.
  ...
  overlay.querySelector("strong").textContent = "Departure noted";
  overlay.querySelector("span").textContent = `${stamp.sign} stamped in passing; the desk was free to be ignored`;
  mechanicalThroat(2);
  /* no preventDefault, no timer: the browser navigates as the visitor chose */
}
